...
`IrregularPlates`, `ContinentalGroups`, `PlateResponse` and `TectonicTerrain` provide the [v4 Minecraft-native candidate](../research/minecraft-native-tectonic-terrain.md): warped weighted plate edges, varied size/age, spatial crust, bounded families of1–4 plates, motion-derived relief and convergent mountain sutures. Size1 is256 blocks high with sea Y63; sizes2–4 are exact integral upscales. The native preset averages29.43% land. v1/v2 and the model-metre v3 remain historical research. The broad response is a bounded kinematic heuristic, not exact contact mechanics or geological history.
