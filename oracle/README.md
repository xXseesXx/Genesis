...
`IrregularPlates`, `ContinentalGroups`, `PlateResponse` and `TectonicTerrain` provide the [Minecraft-native candidate](../research/minecraft-native-tectonic-terrain.md), currently `tectonic-terrain-v8`: warped weighted plate edges, varied size/age, spatial crust, bounded families of 1–4 plates, motion-derived relief and convergent mountain sutures. Size 1 is 256 blocks high with sea Y63; sizes 2–4 are exact integral upscales. The v8 preset uses 2,048-block plate spacing, derivative-damped quintic fBm detail and a linear above-sea height transfer. The broad response is a bounded kinematic heuristic, not exact contact mechanics or geological history.
