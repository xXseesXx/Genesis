...
`IrregularPlates`, `ContinentalGroups`, `PlateResponse` and `TectonicTerrain` provide the [Minecraft-native candidate](../research/minecraft-native-tectonic-terrain.md), currently `tectonic-terrain-v7`: warped weighted plate edges, varied size/age, spatial crust, bounded families of 1–4 plates, motion-derived relief and convergent mountain sutures. Size 1 is 256 blocks high with sea Y63; sizes 2–4 are exact integral upscales. The v7 preset uses 2,048-block plate spacing, four times smaller in X/Z than the immediately prior 8,192-block preset, while retaining the v6 concave above-sea height fit. The broad response is a bounded kinematic heuristic, not exact contact mechanics or geological history.

`WindField`, `ClimateField` and `TerrainSubstrate` form the [coupled climate and ground stage](../research/climate-ground-hydrology.md). An analytic stream function gives a continuous, divergence-free, non-stagnating wind over the entire coordinate plane. Twenty-four fixed semi-Lagrangian passes transport humidity from maritime recharge across the pre-incision terrain; directional uplift enhances windward rainfall and moisture loss creates lee shadows. Five seeded near-surface rock classes expose separate resistance, permeability and weatherability. Humidity, slope and material properties produce soil/rockhead depth plus exact annual runoff, infiltration and evapotranspiration permille shares.

`ContinentalHydrology` v4 adds a separate [complete-family coarse hydrology experiment](../research/continental-hydrology.md): variable irregular active footprints on a canonical world lattice, prescribed maritime reservoirs, terrain-aware precipitation, material-partitioned effective runoff, filled spill levels, steepest descent outside flats and exact source/flux ledgers. Production viewer/adapter construction supplies `ClimateField` and `TerrainSubstrate`; the older uniform-rain constructors deliberately retain all-rain surface-runoff semantics for compatibility fixtures. `ActiveHydrology.solveD8Rivers` adds the steepest-receiver pass without changing the older reference solvers. Family IDs identify work/cache regions, not individual catchments. Independence relies on the terrain's ocean-separated geography; this is not a generic global root provider, transient lake simulation, groundwater model or completed fine-drainage proof.

`FluvialNetwork` v2 derives contributing area/order, connected flat lake components and bounded discharge-sized channel geometry from the final post-erosion DAG. Lake cells do not receive artificial minimum-slope hydraulic profiles; inlet stages blend to the exact receiving-lake surface. Point queries remain fixed work over a 5-by-5 coarse neighborhood and bounded curve chords. `HydraulicErosion` and `MassWasting` retain fixed pass counts, and the adapter realizes the same immutable lake/channel objects after channel carving rather than testing lakes against the pre-channel surface.
