...
Next define broad crustal/continental reference elevation and a fixed world-configuration sea-level policy; assess continental extent and the 50-55% total-land preference over multiple seeds. Then expose this candidate in a separate experimental tectonic viewer, with all field controls in the top bar and no misleading claim of completed rivers. Do not normalize to viewport land fraction. Existing production viewers remain available.
