...
The [one-step crest mesh](../research/crest-mesh-experiment.md) now preserves coarse minimax spill levels and total mass under fine D8 routing, including the three real paired fixtures. This does NOT preserve every receiver or tied outlet share. Next compose the plate-vector profiles with shared junction constraints on a finer surface, preserving or explicitly revising coast/terminal/crest commitments; closest-edge switching is not finished terrain. Add outlet/port-preserving refinement contracts before promoting this to a river cascade. Larger groups, tectonic history/material coupling and full refinement remain open; R2/R3/R4 are not complete.
