...
The user's updated direction is **much fewer plates, usually one and at most five per continent, mostly mixed land/submerged plates, irregular edges, varied sizes, datum and tilt**. The [v2 sparse-plate recipe](../research/sparse-irregular-plates.md) implements warped weighted ownership, default spacing524288 (1/64 the v1 nominal density), size/young-fragment classes, spatial crust lobes, exact families of1–4 plates, and joined datum/tilt. A deep bed band at family frontiers preserves the plate budget even under extreme relief and sea-level settings. Six wide-area samples average50.94% land, with93–99% of well-sampled plates containing both land and water.

The [separate tectonic viewer](Tectonic-Viewer.md) exposes this Java21 candidate at `/tectonics.html`:26 top-bar fields,17 parameters, inspection, navigation and export. Saved v1 links migrate visibly to the new preset. HTTP/common-point and application-logic tests cover the integration; code-generated terrain and plate comparisons were inspected. Browser visual/gesture QA remains outstanding.

Next improve mountain belts/interior relief and less cellular coasts within the small plate-budget requirement. The new family separation is an explicit geographic constraint: larger spacing alone does not remove its wide-scale ocean-network bias. Prefer richer shapes/broad basins over shrinking water margins until continents accidentally join. Follow with actual curved-edge motion normals, smooth mixed-crust forcing and fracture ancestry. Keep below-sea-level terrain distinct from certified hydrology terminals; do not promote family IDs into drainage roots. Retain coordinate/configuration determinism and fixed sea level.
