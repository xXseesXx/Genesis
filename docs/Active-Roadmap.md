...
R1a/R1b are tested finite accounting building blocks. R2a and R2b now include separation, failed-retrofit metrics, and connected authored-land fixtures with independently verified reserve-water connection. Next: examine adjoining-domain and scale statistics, and specify actual routing terminals/depression policies using these provisional complete-domain inputs. A bounded terrain-first coarse routing experiment can use the fixtures without promoting their geography; R2 completion and production choice still require continental extent/topology acceptance. Constructive watershed refinement remains the alternative. Do not reuse growth parents, nearest-sea search or clipping artifacts as a finished river solution.
