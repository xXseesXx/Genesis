...
The [separate tectonic viewer](Tectonic-Viewer.md) now exposes this exact Java21 candidate at `/tectonics.html`:17 top-bar fields, full configuration/coordinate inspection, navigation and export. Candidate crust/regime diagnostics align with the new terrain; old production models remain available. HTTP/common-point and application-logic tests cover the integration. Browser visual/gesture QA is still outstanding because no browser surface was available.

Next evaluate the candidate in the viewer: improve broad mountain-belt/interior relief and coast morphology without reinstating independent noise as the continental organizer. Assess the effect of changing gains/widths on actual height and coast, and retain full coordinate/configuration determinism. Do not normalize to viewport land fraction. The viewer is a research instrument, not an architecture acceptance decision.
