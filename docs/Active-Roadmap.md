...
The separate [active D4 crest solver](../research/paired-drainage-experiment.md) now supports excluded cells, explicit terminals, heterogeneous runoff and finite shared saddle heights. Three unrestricted paired solves equal local solves for spill/receivers/flux, while a lowered-pass counterexample breaks independence as required. Next realize those crests in a shared finer terrain mesh, specify diagonal connectivity, and independently test terrain-based routing and spills. Current crest edges are physical graph constraints, not a finished surface; D4 isolation does not prove D8 safety. Move toward geology-informed forcing (collision, subduction, rifting, transform motion) without equating every plate boundary with a divide. Larger groups and full refinement remain open; R2/R3 are not complete.
