...
R1a/R1b are tested finite accounting building blocks. [R2a](../research/root-terminal-design.md) now tests bounded maritime-domain ownership and quantifies its cost on existing continents; neither complete root candidate is selected. Next is R2b: compare continental authoring with maritime reservations known in advance, across multiple seeds and high-coverage/merged landmasses, then complete interior-water and terminal contracts. A bounded terrain-first coarse solve is the first proposed R3 baseline after geography acceptance; constructive watershed refinement remains the alternative to compare. Do not silently reuse nearest-sea search or clipping artifacts as a finished solution.
