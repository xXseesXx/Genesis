...
| R3 End-to-end watershed proof | Compare both candidates on the same forcing/outlet fixtures; implement bounded coarse solving and nested refinement | No missing boundary inflow, stable ports/corridors, multiple crossings, complete roots; exact matched-model oracle | Building block: active D4 crest solver and three paired union/local comparisons pass; terrain mesh, candidate comparison and nested end-to-end refinement pending |
| R4 Conditioned terrain and material coupling | Reference geology + uplift + discharge/area -> channel profiles, divides, hillslopes, exposed rock | Physical bed descent, stable lake surfaces, independent terrain-drainage agreement, no shortcuts; hardness counterfactual | Reference forcing profiles now use existing plate vectors/crust; integrated terrain/material/water coupling pending |
...
The [plate-vector forcing reference](../research/boundary-forcing-experiment.md) now derives canonical compression/extension/shear and crust-dependent profiles from existing core plates. A controlled rift lowers a pass enough to invalidate the old drainage divide, detected by the unrestricted solve. Next compose nearby profiles with shared junction constraints and realize them on the finer terrain/crest mesh; closest-edge switching is not continuous terrain. Preserve or explicitly revise coast/outlet/budget commitments, then independently test diagonal routing and saddle spills. The active D4 paired checks remain the reference baseline. Larger groups, tectonic history/material coupling and full refinement remain open; R2/R3/R4 are not complete.
