...
The [separate tectonic viewer](Tectonic-Viewer.md) exposes this Java21 candidate at `/tectonics.html`:27 top-bar fields,18 world parameters and separate contour spacing, inspection, navigation and export. Saved v1/v2 links migrate visibly to the new preset. Fixed high-contrast colors, requested5–1000 m contours and reported zoom-dependent line LOD improve elevation reading. HTTP/contour-crop/common-point and application-logic tests cover integration; code-generated height and mountain-detail images were inspected. Browser visual/gesture QA remains outstanding.

Next replace approximate site normals/proximity loading with actual curved-contact information, smooth thresholded mixed-crust forcing, and improve mountain interiors and less cellular bounded-family coast shapes. Broad plate response is currently a local kinematic heuristic, not a mechanical/history solver. Family separation is an explicit geographic constraint: larger spacing or lowering alone does not remove its ocean-network bias. Keep below-sea-level terrain distinct from certified hydrology terminals; do not promote family IDs into drainage roots. Retain coordinate/configuration determinism and fixed sea level. Fracture ancestry and drainage-preserving realization remain subsequent work.
