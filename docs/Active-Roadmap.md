...
Implement the [explicit active-domain drainage contract](../research/authored-mosaic-experiment.md#next-executable-step-explicit-active-domain-drainage): excluded neighbor interiors, supplied integer elevations/net runoff, certified ocean terminals, explicit overflow/depression policy, unresolved closed components, independent minimax and mass checks. The existing finite oracle cannot represent excluded cells or general source arrays; extend carefully or add a separate experiment. Use provisional complete-domain inputs without promoting their rejected geography. Stop coverage/noise tuning on the cellular-island layout. R2 completion still requires acceptable continental/root architecture; constructive inland watershed refinement remains the alternative. R3 is not complete until matched-model end-to-end routing and refinement evidence exists.
