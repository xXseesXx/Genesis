...
The current [v4 native recipe](../research/minecraft-native-tectonic-terrain.md) retains sparse irregular ownership,1–4-plate families and motion-driven sutures while replacing model metres with Minecraft blocks. Size1 is256 blocks high at sea Y63 and defaults to65,536-block plates; sizes2–4 exactly upscale topology and hydrology. Six wide samples average29.4342% land. Inter-family ocean margins still preserve the continent-size cap; this is not a claim that every plate boundary is freely connectable.
...
The [tectonic viewer](Tectonic-Viewer.md) now has33 top-bar fields,20 world parameters, block contour spacing, exact inspection/navigation/export, fullscreen map mode and a separate hydrology version. Terrain v4 is Minecraft-native: size1 is256 blocks high with sea Y63; sizes2–4 exactly upscale topology, surface height and hydrology support. Uniform rainfall defaults to1000 model mm/year and is replaceable via a coordinate-local provider. Code-generated terrain/height/river maps inspected; full-continent support, mass/source walks, native-scale counterfactuals, cache/concurrency and nonzero-river crop/zoom checks pass. Fine channels and the GTNH adapter remain deferred.
