...
Use the new [mixed ridge/ocean paired inputs](../research/mixed-boundary-experiment.md) for explicit-active-graph routing, supplied elevations/net runoff, ocean terminals and depression overflow. Implement a shared crest and independently test physical divide preservation, including diagonal shortcuts and spill levels; simply blocking cross-owner edges is not a terrain proof. The existing finite oracle has no active mask/general source arrays; extend carefully or add a separate experiment. Disjoint pairs preserve a bounded first topology fixture, not final continental extent. Grow to larger connected groups only with explicit junction/ocean/lookup contracts. R2/R3 remain open until physical routing, complete budgets, geography and refinement are validated together.
