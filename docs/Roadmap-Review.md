...
User clarification: the world target is infinite and non-repeating, fully deterministic regardless of generation order. Finite/wrapped-map alternatives are no longer open design options. The ±2^40 guard remains only numeric support, not a terminal boundary. The old JS prototype is unavailable and no longer a dependency. Current progress and remaining root/refinement contracts are tracked in [canonical drainage](Canonical-Drainage.md).
