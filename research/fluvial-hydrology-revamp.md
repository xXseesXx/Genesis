...
The current checkpoint composes `tectonic-terrain-v8`, `terrain-climate-v1`, `wind-field-v1`, `terrain-substrate-v1`, `continental-hydrology-v4`, `fluvial-network-v4`, `continental-erosion-v2`, `mass-wasting-v1`, adaptive hydrology v2, and `genesis-columns-v8` as a deliberately bounded subset of this design:
...
- The final route and lake levels are solved after incision and mass wasting. Lake cells receive no fake minimum-slope hydraulic profile except the component's real, discharge-sized spill reach. That outlet centerline starts at the spill cell rather than one downstream cell; viewer and adapter composition suppress its river banks/current while it remains beneath the lake surface. Incoming river water blends to the exact receiving-lake stage. The adapter carves the channel before testing lake open volume, so a deep inlet is back-filled correctly, while a fractional shoreline fringe is not excavated solely to force one voxel.
