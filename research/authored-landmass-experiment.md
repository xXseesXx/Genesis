...
Subsequent result: the [adjoining-domain/scale audit](authored-mosaic-experiment.md) exposes the cellular-ocean signature and rejects this exact layout as production continents. Retain the connected fixture for explicit active-domain routing/terminal/depression experiments; stop coverage/noise tuning on this layout. The root architecture remains open until continental extent and topology are acceptable. Production geography must not be changed merely because the fixture's tests pass.
