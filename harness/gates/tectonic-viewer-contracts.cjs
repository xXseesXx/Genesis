...
async function run(){
  ...
  assert.match(html,/id="liveHydrology"[^>]*checked/,'Live map updates must be enabled by default');assert.match(html,/class="viewer-layout"/,'Controls and map need a side-by-side desktop layout');
  class Element{
  ...
  const dendriticLayers=['dendriticTerrain','dendriticDelta','dendriticRidges'];
  ...
  assert.deepEqual(dendriticLayers.filter(id=>!evaluate('state.meta.fields.some(field=>field.id==='+JSON.stringify(id)+')')),[],'Missing dendritic viewer fields');
  assert.deepEqual(dendriticLayers.filter(id=>!evaluate('HYDRO_LAYERS.has('+JSON.stringify(id)+')')),[],'Dendritic fields must use bounded hydrology tiles');
  ...
  assert.match(context.savedURL,/plateWarpPermille=220/);assert.equal(evaluate('Object.keys(state.committed.params).length'),51);
  assert.equal(elements.get('hydrologyParams').children.length,6,'Hydrology controls must be grouped by process');assert.equal(elements.get('p-massWasting').checked,true);assert.equal(elements.get('p-braidedChannels').checked,true);assert.equal(elements.get('r-rainfallMm').value,1000);
  assert.match(context.savedURL,/version=tectonic-terrain-v8/);assert.doesNotMatch(context.savedURL,/crustProvinceScale/);
  ...
  assert.match(context.savedURL,/hydrologyVersion=continental-hydrology-v6/);assert.match(context.savedURL,/fluvialVersion=fluvial-network-v5/);assert.match(context.savedURL,/climateVersion=terrain-climate-v1/);assert.match(context.savedURL,/windVersion=wind-field-v1/);assert.match(context.savedURL,/substrateVersion=terrain-substrate-v1/);assert.match(context.savedURL,/dendriticVersion=dendritic-erosion-v2/);assert.equal(evaluate('state.params.rainfallMm'),1000);
  ...
  elements.get('layers').children[0].click();await settled();assert.equal(evaluate('state.committed.params.forcingPermille'),1450);assert.equal(elements.get('p-forcingPermille').value,'0');
  ...
  elements.get('export').click();assert.equal(downloads.length,2);const config=JSON.parse(await blobs.get(downloads[1].url).text());assert.equal(config.model,'tectonic-experimental');assert.equal(config.seed,'-9223372036854775808');assert.equal(config.layer,'base');assert.equal(config.params.forcingPermille,0);assert.match(config.waterStatus,/bounded fine channel corridors/);assert.match(config.waterStatus,/connected-lake/);assert.equal(config.hydrologyVersion,'continental-hydrology-v6');assert.equal(config.fluvialVersion,'fluvial-network-v5');assert.equal(config.climateVersion,'terrain-climate-v1');assert.equal(config.windVersion,'wind-field-v1');assert.equal(config.substrateVersion,'terrain-substrate-v1');assert.equal(config.dendriticVersion,'dendritic-erosion-v2');
  ...
  elements.get('reset').click();await settled();assert.equal(evaluate('state.seed'),'42');assert.equal(evaluate('state.params.plateSpacing'),2048);assert.equal(evaluate('state.params.plateWarpPermille'),220);assert.equal(evaluate('state.params.landHeight'),64);assert.equal(evaluate('state.params.forcingPermille'),1450);assert.equal(evaluate('state.params.detailHeight'),10);assert.equal(evaluate('state.params.plateRoughnessPermille'),170);assert.equal(evaluate('state.params.plateRelief'),18);assert.equal(evaluate('state.params.plateTilt'),24);
  ...
  elements.get('liveHydrology').checked=true;elements.get('r-rainfallMm').value='1250';elements.get('r-rainfallMm').oninput();assert.equal(elements.get('p-rainfallMm').value,'1250','Hydrology slider and exact number input must stay linked');elements.get('worldForm').events.input({target:elements.get('r-rainfallMm')});await new Promise(r=>setTimeout(r,400));await settled();assert.equal(evaluate('state.committed.params.rainfallMm'),1250,'Enabled live controls must render without Apply');
  elements.get('p-massWasting').checked=false;elements.get('worldForm').events.submit({preventDefault(){}});await settled();assert.equal(evaluate('state.committed.params.massWasting'),0);assert.match(context.savedURL,/massWasting=0/);
  ...
  assert.equal(evaluate('state.params.plateSpacing'),2048);assert.equal(evaluate('state.x'),-4096);assert.equal(evaluate('state.z'),2048);assert.equal(evaluate('state.step'),32);assert.match(context.savedURL,/version=tectonic-terrain-v8/);assert.match(context.savedURL,/hydrologyVersion=continental-hydrology-v6/);assert.match(context.savedURL,/climateVersion=terrain-climate-v1/);
  await evaluate("location.search='?version=tectonic-terrain-v8&hydrologyVersion=continental-hydrology-v6&fluvialVersion=fluvial-network-v5&climateVersion=terrain-climate-v1&windVersion=wind-field-v1&substrateVersion=terrain-substrate-v1&plateSpacing=2048&x=-3071&z=3073&step=17';init()");await settled();
  ...
  console.log('PASS TECTONIC VIEWER LOGIC: v8 derivative terrain and compact preset, climate/ground layers, robust inspector, debounced/cached hover, live drag, bounded tiles, zero-request revisit, in-flight reuse, clipping, fullscreen and export');
}
