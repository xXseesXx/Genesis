...
async function run(){
  ...
  const context=vm.createContext({document:{body,getElementById:id=>{assert(elements.has(id),'Missing DOM node '+id);return elements.get(id);},createElement:tag=>new Element(tag)},
    location:{search:'?seed=-9223372036854775808'},history:{replaceState(_a,_b,url){context.savedURL=url;}},URL:LocalURL,URLSearchParams,AbortController,Blob,setTimeout,clearTimeout,
  ...
  await settled();assert.equal(evaluate('state.committed.seed'),'-9223372036854775808');assert.equal(elements.get('layers').children.length,17);
  assert.match(context.savedURL,/crustProvinceScale=4/);assert.equal(evaluate('Object.keys(state.committed.params).length'),14);
  ...
  await evaluate('inspect({x:2,z:3})');assert.equal(elements.get('inspection').children.length,34);assert.doesNotMatch(elements.get('inspectCoords').textContent,/loading/);
  ...
  elements.get('seed').value='42';elements.get('p-crustProvinceScale').value='2';elements.get('worldForm').events.submit({preventDefault(){}});assert.match(elements.get('error').textContent,/1 or 4/);
  elements.get('reset').click();await settled();assert.equal(evaluate('state.seed'),'42');assert.equal(evaluate('state.params.crustProvinceScale'),4);
  assert(requests.filter(x=>x.startsWith('/api/tectonic/render')).every(x=>x.includes('continentalPercent=')&&x.includes('seaLevel=')&&x.includes('crustProvinceScale=')));
  console.log('PASS TECTONIC VIEWER LOGIC: 17 layers, full configuration/exact seed, navigation, inspector, stale response isolation, export and invalid inputs; not browser visual QA');
}
