...
const LIMIT=2**40,INPUTS=['x','z','step','seed','layer','version','hydrologyVersion','fluvialVersion','climateVersion','windVersion','substrateVersion','contourInterval'];
...
function versionKey(frame){return [state.meta.model,state.meta.version,frame.hydrologyVersion,frame.fluvialVersion,frame.climateVersion,frame.windVersion,frame.substrateVersion].join('|');}
...
const TILE_SIZE=128,MIN_HYDRO_TILE=64,TILE_LIMIT=128,TILE_WORKERS=4,
  HYDRO_LAYERS=new Set(['erodedTerrain','erosionDepth','hardness','riverMap','rainfall','humidity','runoff','lakeDepth','waterSurface','soilDepth','bedrockElevation','bedrockDepth','rockType','infiltration','runoffFraction','drainage','drainageStatus']),
...
async function render(){
  ...
  try{
    ...
    const saved=query(frame);saved.delete('width');saved.delete('height');saved.set('version',state.meta.version);for(const key of ['hydrologyVersion','fluvialVersion','climateVersion','windVersion','substrateVersion'])saved.set(key,frame[key]);history.replaceState(null,'','/tectonics.html?'+saved);
...
async function inspect(pixel){
  ...
  try{const response=await fetch('/api/tectonic/sample?'+query(frame,point));if(!response.ok)throw new Error((await response.json()).error||'Inspection failed');const data=await response.json();
    if(revision!==state.inspectRevision)return;if(data.model!==state.meta.model||data.version!==state.meta.version||data.hydrologyVersion!==frame.hydrologyVersion||data.fluvialVersion!==frame.fluvialVersion||data.climateVersion!==frame.climateVersion||data.windVersion!==frame.windVersion||data.substrateVersion!==frame.substrateVersion||data.seed!==frame.seed||data.x!==String(point.x)||data.z!==String(point.z)||data.size!==frame.params.size)throw new Error('Inspector returned a mismatched world point.');
...
function hoverMove(e){
  ...
  hoverTimer=setTimeout(async()=>{
    ...
    try{
      ...
      if(data.version!==state.meta.version||data.hydrologyVersion!==frame.hydrologyVersion||data.fluvialVersion!==frame.fluvialVersion||data.climateVersion!==frame.climateVersion||data.windVersion!==frame.windVersion||data.substrateVersion!==frame.substrateVersion||data.seed!==frame.seed||data.x!==String(point.x)||data.z!==String(point.z))throw new Error('Terrain details out of date');
...
$('reset').onclick=()=>{if(!state.meta)return;for(const s of state.meta.params)$('p-'+s.id).value=s.default;$('seed').value='42';$('originX').value=DEFAULT_VIEW.x;$('originZ').value=DEFAULT_VIEW.z;$('step').value=DEFAULT_VIEW.step;state.contourInterval=5;$('contourInterval').value=5;apply();};
...
async function init(){
  try{$('upgradeNotice').hidden=true;const response=await fetch('/api/tectonic/meta');if(!response.ok)throw new Error('Tectonic API unavailable. Restart the updated local server.');state.meta=await response.json();
    ...
    if(savedVersion==='tectonic-terrain-v4')migrateCompactScale(url,32,'This v4 link used the original model-metre terrain. Its location, sampling step and plate spacing were divided by 32 exactly once for the Minecraft-scale v7 world; compatible settings are preserved.');
    if(savedVersion==='tectonic-terrain-v5'||savedVersion==='tectonic-terrain-v6')migrateCompactScale(url,4,'This saved link predates the fourfold-smaller v7 world. Its location, sampling step and plate spacing were divided by 4 exactly once; compatible settings are preserved.');
    ...
    $('params').replaceChildren();$('layers').replaceChildren();
    ...
    $('version').textContent=[state.meta.version,state.meta.hydrologyVersion,state.meta.fluvialVersion,state.meta.climateVersion,state.meta.windVersion,state.meta.substrateVersion].join(' · ');apply();
...
function migrateCompactScale(url,divisor,message){
  ...
  for(const key of ['version','hydrologyVersion','fluvialVersion','climateVersion','windVersion','substrateVersion'])url.delete(key);$('upgradeNotice').textContent=message;$('upgradeNotice').hidden=false;
}
init();
