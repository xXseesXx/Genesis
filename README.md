...
Follow the [active roadmap](docs/Active-Roadmap.md): [existing plate vectors now drive shared reference profiles](research/boundary-forcing-experiment.md), with a forcing-induced drainage leak caught by the active solver. Next: multi-edge/junction composition on a shared finer terrain mesh and diagonal routing safety. Paired land still targets 53% total area; closest-edge reference profiles are not final elevation. No production terrain/viewer fields changed. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
