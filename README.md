...
- TECTONIC TERRAIN V2: sparse irregular plates with varied size/age, spatial continental/oceanic terrain, 1–4 plate continental families, and individual datum/tilt. Tests cover 288 complete-field larger-support/cold cases, reciprocal groups, extreme physical ocean margins, concurrent/counterfactual sampling and six area/morphology audits. Wide samples average 50.94% land. See [construction and limitations](research/sparse-irregular-plates.md).
- TECTONIC VIEWER: 26 top-bar layers with HTTP/direct/crop/zoom and exact inspector/configuration checks. Node tests cover navigation, stale responses, exports and migration of saved v1 links. Browser visual QA is not claimed. See [viewer contracts](docs/Tectonic-Viewer.md).
...
Open the [tectonic terrain viewer](http://127.0.0.1:8787/tectonics.html): sparse irregular plates, predominantly single-plate continental families, mixed submerged terrain, and plate datum/tilt, with 26 top-bar fields. Follow the [active roadmap](docs/Active-Roadmap.md) for mountain/interior/coast refinement and unresolved hydrology. The page remains a Java21 experiment. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
