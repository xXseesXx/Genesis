...
- TECTONIC TERRAIN V4: Minecraft-native size1 uses a256-block column and sea Y63; sizes2–4 exactly upscale seeded horizontal features, relief and drainage. Sparse irregular plates, bounded1–4-plate families and motion-driven mountain joins remain. The native preset averages29.43% land across six wide samples. See [the native scale contract](research/minecraft-native-tectonic-terrain.md).
...
- TECTONIC VIEWER:33 top-bar layers,20 world parameters, native world-height/sea-level reporting, rainfall/river/lake diagnostics, block contours and a512² fullscreen map mode. HTTP tests cover size2 output, river crop/zoom, exact fluxes and dry rain; application tests cover fullscreen transitions, configuration, navigation, stale responses and exports. See [viewer contracts](docs/Tectonic-Viewer.md).
