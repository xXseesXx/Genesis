...
Follow the [active roadmap](docs/Active-Roadmap.md): the first finite boundary-summary experiment is complete, and conservative basin-compatible runoff allocation is next. Then compare explicit root/terminal constructions before integrating full watershed refinement and conditioned terrain. The [research](research/README.md) distinguishes these finite building blocks from the unresolved infinite-plane contracts. Independent geology remains useful but must not bypass those dependencies. No terrain/viewer fields changed in R1a. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) summarizes the implementation and constraints.
