...
Per the latest direction, advance independent geology work (faults/intrusions, explicit input fixtures and scenario loading) before returning to the harder water work where practical. M5a supplies layered material queries; it does not finish geology or justify skipping hydrology dependencies in erosion, climate-fed runoff and sediment transport. Water still needs multilevel conditioned refinement, infinite-plane terminal/root ownership—especially where continents merge—and actual downhill channel/valley shaping. Natural coastline detail can evolve separately while preserving coarse commitments. The [roadmap review](docs/Roadmap-Review.md) tracks the outstanding water contracts. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) summarizes the current implementation and constraints.
