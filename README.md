...
Follow the [active roadmap](docs/Active-Roadmap.md): the [multi-domain audit](research/authored-mosaic-experiment.md) rejects the current cellular-island layout for production. Connected authored land remains a provisional complete-domain drainage fixture. Next: implement explicit active-cell routing, ocean terminals, depression overflow and independent mass/spill checks; do not keep tuning coverage to hide a structural ocean network. Actual inland divides and continental extent remain open. No production terrain/viewer fields changed. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
