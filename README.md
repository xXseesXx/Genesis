...
Follow the [active roadmap](docs/Active-Roadmap.md): the [absolute tectonic candidate](research/tectonic-terrain-experiment.md) now has correlated continental crust, shared motion-driven relief and a fixed-sea-level coast. Next: expose that exact model in a separate experimental viewer with top-bar fields. Global rivers remain an independent open contract: even smooth blending can invalidate a divide, and the [crest mesh](research/crest-mesh-experiment.md) does not preserve every outlet. No production terrain/viewer fields changed in this checkpoint. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
