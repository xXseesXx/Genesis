...
- TECTONIC VIEWER:27 top-bar layers including high-contrast height/contours,18 world parameters and separate display spacing. HTTP tests cover direct/crop/unadorned-zoom equality and contour halos/LOD; Node checks configuration, navigation, stale responses, exports and legacy-link migration. Browser visual QA is not claimed. See [viewer contracts](docs/Tectonic-Viewer.md).
...
Open the [tectonic terrain viewer](http://127.0.0.1:8787/tectonics.html): sparse irregular plates, about30% default land, motion-driven datum/tilt, mountain joins and27 top-bar fields including **Height + contours**. Follow the [active roadmap](docs/Active-Roadmap.md) for contact-aware relief, coast refinement and unresolved hydrology. The page remains a Java21 experiment. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
