...
- TECTONIC TERRAIN: correlated crust supplies a common base/forcing/height/coast recipe at fixed sea level;540 larger-support cases,480 province checks, exact-output/cold/concurrent/counterfactual tests and six morphology/wide-area audits. Writes `build/tectonic-terrain.json` and three inspected images. Wide samples yield51.4-53.9% land; final geography and hydrological roots remain open.
- TECTONIC VIEWER:17 candidate layers with real HTTP/direct/crop/zoom and exact inspector/configuration checks. `/tectonics.html` adds top-bar controls, navigation and export; Node application-logic tests cover stale responses and export snapshots in CI. Browser visual QA is not claimed. See [viewer contracts](docs/Tectonic-Viewer.md).
...
Open the new [tectonic terrain viewer](http://127.0.0.1:8787/tectonics.html): correlated continental crust, shared motion-driven relief and a fixed-sea-level coast, with17 top-bar fields. Follow the [active roadmap](docs/Active-Roadmap.md) for mountain/interior/coast refinement and unresolved hydrology. Old production models remain unchanged; the new page is a Java21 experiment, not final architecture. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
