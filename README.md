...
Follow the [active roadmap](docs/Active-Roadmap.md): finite boundary summaries and [catchment-aligned conservative runoff](research/conservative-runoff-experiment.md) are tested building blocks. Next, compare explicit root/terminal constructions and balanced ownership before integrating full watershed refinement and conditioned terrain. The [research](research/README.md) separates exact finite accounting from unresolved infinite-plane, climate and quantization contracts. Independent geology remains useful but must not bypass those dependencies. No terrain/viewer fields changed in R1a/R1b. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) summarizes the implementation and constraints.
