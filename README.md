...
Follow the [active roadmap](docs/Active-Roadmap.md): [mixed ridge/ocean boundaries](research/mixed-boundary-experiment.md) now join land across paired cells and target 53% of total owned area, within the user's 50-55% preference. Next: explicit active-cell routing, physically shared crests, ocean terminals, depression overflow and independent mass/spill/no-leak checks. The two-cell grouping is a bounded first fixture, not accepted continent architecture. No production terrain/viewer fields changed. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
