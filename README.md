...
Follow the [active roadmap](docs/Active-Roadmap.md): finite boundary/source accounting is tested, and [R2 root/terminal research](research/root-terminal-design.md) now includes a bounded maritime-envelope experiment. Late clipping visibly cuts existing continents, so it is not adopted. Next, compare geography authored with maritime reservations already known, then complete terminal contracts before the end-to-end watershed solve. The [research](research/README.md) separates measured building blocks from open infinite-plane, climate and geometry guarantees. No production terrain/viewer fields changed in these research stages. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) summarizes the implementation and constraints.
