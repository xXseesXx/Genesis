...
Follow the [active roadmap](docs/Active-Roadmap.md): finite accounting and bounded maritime lookup are tested, and [connected authored land](research/authored-landmass-experiment.md) now supplies provisional complete-domain fixtures with independent water-connection checks. Late clipping and whole-object rejection remain rejected retrofits. Next: assess adjacent-domain/scale bias and specify actual river terminals/depression handling on the new fixtures. The growth tree is not a river graph, and the single-landmass/domain assumption is not accepted world geography. No production terrain/viewer fields changed. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
