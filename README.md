...
The **M0 foundation, M1 tectonics, and M2a coast/elevation prototype are implemented**, with M3a canonical drainage-geometry groundwork. Twenty-two fields expose plate structure, crust blending, committed land/sea signs, macro elevation, bounded coarse drainage, viewport-independent guide/port distances, and a separate continental scaffold candidate. The target is an **infinite, non-repeating world, deterministic regardless of generation order**. Global ocean connectivity and the complete hydrology cascade remain open; Minecraft integration remains deferred. See [tectonics](docs/Tectonics.md), [coasts/elevation](docs/Coasts-and-Elevation.md), and [canonical drainage](docs/Canonical-Drainage.md) for contracts and limitations.

**Continental overview** on the main page selects the new connected-landmass candidate at continental scale. Its two layers lead the top bar; four new shape controls are in the parameter registry. It is intentionally separate from existing elevation/drainage, and still visibly rounded before coastline refinement. See [the scaffold contract](docs/Continental-Scaffold.md) and code-generated `build/gallery/continental-comparison.png` for an old/new comparison.
...
- DET: all 22 fields over a 64×64-chunk region in raster, shuffled cached, and cold per-chunk order; additional extreme seeds/coordinates.
