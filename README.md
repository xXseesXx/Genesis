...
Follow the [active roadmap](docs/Active-Roadmap.md): [active paired drainage](research/paired-drainage-experiment.md) now conserves runoff and validates finite crest barriers through unrestricted union/local comparisons, including a deliberate lowered-pass failure. Next: shared finer terrain, diagonal routing safety and geology-informed forcing. Paired land still targets 53% total area; neither the two-cell grouping nor synthetic heights are final continent architecture. No production terrain/viewer fields changed. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
