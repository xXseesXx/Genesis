...
The `experiment/dendritic-terrain` fork adds a separately selectable **Genesis
Dendritic** world type. It applies a continuous, drainage-guided four-scale
erosion-morphology field to mountains, then lets the existing canonical river
and lake solve carve/fill the result. The classic **Genesis** world type is
unchanged. See the [design, continuity contract and limits](research/dendritic-terrain.md).

The experimental [Minecraft adapter](mc-adapter/README.md) builds a selectable **Genesis** world type from the supplied GTNH starter. Its current path is terrain `tectonic-terrain-v8`, climate `terrain-climate-v1`, substrate `terrain-substrate-v1`, hydrology `continental-hydrology-v4`, fluvial interpretation `fluvial-network-v4`, erosion v2 and mass wasting v1. From the repository root, build with `.\mc-adapter\gradlew.bat build`; Java 21 and a modern-Java GTNH runtime are required. JVM adapter tests pass, but in-game GTNH behavior and saved-world versioning remain unverified; use a new world because old chunks are not regenerated.
...
Hydrology v4 owns the shared `fluvial-network-v4`: accumulated effective runoff yields distinct mean/bankfull discharge, width, depth and velocity, while contributing area, Strahler order, slope and material resistance inform reach form. Minecraft channel widths are scaled eightfold at the hydraulic-profile level while discharge remains conserved, producing broader and generally shallower rivers. Bounded curved reaches select cascade, straight, meandering or two-thread braided geometry. Face-connected depressions expose flat lake surfaces and stable spill/outlet records after incision and gravity erosion; inlet water profiles meet the receiving lake stage, while each discharge-sized outlet starts at its lake spill node and carries no separate river border across the lake surface. The viewer and adapter consume the same immutable objects. See the [research-backed fluvial target and status checklist](research/fluvial-hydrology-revamp.md).

Adapter columns v10 rasterize each chunk from a shared one-block world-coordinate halo. The final voxel pass closes every dry cardinal face beside ocean, lake or river water at the adjacent integer stage, so chunk order and chunk borders cannot create a different shoreline or a vanilla-fluid escape gap. Its stronger terrain preset adds greater elevation contrast and steeper plate-boundary ridges; exposed resistant-rock river drops can retain an upstream water stage over the lower bed to form rasterized waterfalls.

Terrain v8 replaces the concave hyperbolic height amplifier with a linear native-height transfer and uses derivative-damped quintic fBm for terrain detail. It retains the **2,048-block plate spacing**, 256-block height and sea Y63. The hydrology lattice remains 32 blocks while the adapter refines ocean and lake water masks to two blocks.
...
- TECTONIC TERRAIN V8: Minecraft-native size 1 uses a 256-block column and sea Y63; sizes 2–4 exactly upscale seeded horizontal features, relief and drainage. The 2,048-block plate preset retains sparse irregular 1–4-plate families and motion-driven mountain joins, with derivative-damped detail and linear native height. See [the native scale contract](research/minecraft-native-tectonic-terrain.md).
