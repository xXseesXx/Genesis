...
Follow the [active roadmap](docs/Active-Roadmap.md): plate-vector profiles and a [crest-preserving fine mesh](research/crest-mesh-experiment.md) now supply reference forcing and D8 spill/mass checks. Next: shared profile junctions and explicit outlet/port-preserving refinement; matching spill levels alone does not preserve a river graph. Paired land still targets 53% total area. No production terrain/viewer fields changed. The unavailable JS prototype is not a prerequisite. Preserve this repository and its `origin`; eventual mod scaffolding belongs under `mc-adapter/` using the GTNH starter. [Session handoff](docs/Session-Handoff.md) preserves the current context.
